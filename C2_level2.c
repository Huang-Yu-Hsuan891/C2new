...
int main(){
    ...
    for (step = 0; step < 1; step++) {
        ...
        while (s < 100 || er2 < 100/*s < 100*/) {
            ...
            /*if(k == 100) */printf("er2 = %d; s = %d; k[%d] = %d\n",er2, s, num, k);
            error1 = 0;
            error2 = 0;
            for(i = 0; i < (n/2); i++) {
            ...
            outfp1 = fopen("C2_LEVEL2_1_5_totalerror_vr2_eb=100_iter_100.txt","a");
            for (i = 0; i < 1; i++) {
                fprintf(outfp1,"snr = %g ",ebn0s[i]);
                fprintf(outfp1,"num = %d ",num);
                fprintf(outfp1,"totalerror1 = %d ",totalerror1);
                fprintf(outfp1,"totalerror2 = %d ",totalerror2);
                fprintf(outfp1,"\n");
            }
            fclose(outfp1);
        }
    ...
    outfp = fopen("C2_LEVEL2_1_5_vr2_eb=100_iter_100.txt","w");
    for (i = 0; i < 1; i++) {
         fprintf(outfp,"%g ",ebn0s[i]);
         fprintf(outfp,"%g ",berscompare[i]);
         fprintf(outfp,"%g ",bers1[i]);
         fprintf(outfp,"%g ",bers2[i]);
         fprintf(outfp,"\n");
    }
    fclose(outfp);
    // close write file
